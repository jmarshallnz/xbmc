...
bool CCueDocument::Parse(const std::string &strFile)
{
  ...
  while (true)
  {
    ...
    if (StringUtils::StartsWithNoCase(strLine,"INDEX 01"))
    {
      ...
      {
        CLog::Log(LOGDEBUG, "Track split over multiple files, unsupported (%s)", strFile.c_str());
        return false;
      }
      ...
      if (time == -1)
      { // Error!
        CLog::Log(LOGDEBUG, "Mangled Time in INDEX 0x tag in CUE file (%s)", strFile.c_str());
        return false;
      }
  ...
  if (m_iTotalTracks >= 0)
    ...
    CLog::Log(LOGDEBUG, "No INDEX 01 tags in CUE file (%s)", strFile.c_str());
  m_file.Close();
  if (m_iTotalTracks >= 0)
  {
    m_iTotalTracks++;
