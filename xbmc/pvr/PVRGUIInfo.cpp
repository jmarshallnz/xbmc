...
void CPVRGUIInfo::UpdateMisc(void)
{
  ...
  std::string strPlayingClientName     = bStarted ? g_PVRClients->GetPlayingClientName() : StringUtils::Empty;
...
{
  if (!strcmp(m_qualityInfo.strServiceName, ""))
    strValue = StringUtils::Format("%s", g_localizeStrings.Get(13205).c_str());
  else
...
{
  if (!strcmp(m_qualityInfo.strMuxName, ""))
    strValue = StringUtils::Format("%s", g_localizeStrings.Get(13205).c_str());
  else
...
{
  if (!strcmp(m_qualityInfo.strProviderName, ""))
    strValue = StringUtils::Format("%s", g_localizeStrings.Get(13205).c_str());
  else
